...
import { getCachedDomains, setCachedDomains, getCachedAliases, setCachedAliases, invalidateAliases, invalidateAll, updateCachedAlias, removeCachedAlias, } from '../src/lib/cache.js';
beforeEach(() => {
    invalidateAll();
});
/* ====== Domain cache ====== */
...
describe('alias cache', () => {
    ...
    it('invalidateAliases clears a specific domain', () => {
        setCachedAliases('example.com', [{ id: '1' }]);
        setCachedAliases('test.com', [{ id: '2' }]);
...
describe('updateCachedAlias', () => {
    ...
    it('returns false when alias not in cache', () => {
        ...
        assert.equal(updateCachedAlias('example.com', '999', { id: '999' }), false);
    });
    it('returns false when domain not cached', () => {
        assert.equal(updateCachedAlias('no-domain.com', '1', { id: '1' }), false);
    });
});
/* ====== removeCachedAlias ====== */
