...
import './setup.js';
import { getAliasType, formatEmail, truncateRecipients, formatDate, friendlyError, splitLines, splitCommas, resolveDomain, matchesAlias, } from '../src/lib/utils.js';
/* ====== getAliasType ====== */
...
describe('truncateRecipients', () => {
    ...
    it('handles non-array input', () => {
        const result = truncateRecipients(null);
...
describe('friendlyError', () => {
    beforeEach(() => {
        globalThis.browser.i18n._clearMessages();
        globalThis.browser.i18n._setMessages({
...
describe('splitCommas', () => {
    ...
    it('handles empty/null input', () => {
        ...
        assert.deepEqual(splitCommas(null), []);
    });
});
/* ====== resolveDomain ====== */
describe('resolveDomain', () => {
    it('returns domain.name from alias object', () => {
        assert.equal(resolveDomain({ domain: { name: 'example.com' } }, 'fallback.com'), 'example.com');
    });
    it('returns string domain from alias object', () => {
        assert.equal(resolveDomain({ domain: 'example.com' }, 'fallback.com'), 'example.com');
    });
    it('returns fallback when alias has no domain', () => {
        assert.equal(resolveDomain({}, 'fallback.com'), 'fallback.com');
    });
    it('returns fallback when alias is null', () => {
        assert.equal(resolveDomain(null, 'fallback.com'), 'fallback.com');
    });
