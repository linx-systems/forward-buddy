...
export function friendlyError(err) {
    if (err.status === 401)
        return browser.i18n.getMessage('errorInvalidToken');
    if (err.status === 429)
        return browser.i18n.getMessage('errorRateLimited');
    if (err.status === 404)
        return browser.i18n.getMessage('errorNotFound');
    if (!navigator.onLine)
        return browser.i18n.getMessage('errorNetwork');
...
export function parseEmailAddress(email) {
    ...
    if (simple)
        return { local: simple[1].toLowerCase(), domain: simple[2].toLowerCase() };
    return null;
}
/**
 ...
 */
export function matchesAlias(localPart, domain, alias) {
    const aliasDomain = resolveDomain(alias);
    if (aliasDomain.toLowerCase() !== domain.toLowerCase())
        return false;
    if (alias.name === '*')
        return true;
